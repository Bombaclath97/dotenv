...
class AsciidocParser {
    constructor(filename, errorCollection = null) {
        this.filename = filename;
        this.errorCollection = errorCollection;
        this.html = '';
        this.document = null;
        this.ext_path = vscode.extensions.getExtension('asciidoctor.asciidoctor-vscode').extensionPath;
        this.stylesdir = path.join(this.ext_path, 'media');
    }
    getAttribute(name) {
        return util_1.isNullOrUndefined(this.document) ? null : this.document.getAttribute(name);
    }
    getMediaDir(text) {
    ...
    convert_using_javascript(text, forHTMLSave, backend) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve) => {
                ...
                const contains_style = !util_1.isNullOrUndefined(text.match(new RegExp("^\\s*:(stylesheet|stylesdir):", "img")));
    ...
    convert_using_application(text, forHTMLSave, backend) {
        return __awaiter(this, void 0, void 0, function* () {
            const editor = vscode.window.activeTextEditor;
            const doc = editor.document;
            ...
            return new Promise((resolve) => {
                ...
                var options = { shell: true, cwd: path.dirname(this.filename), env: Object.assign(Object.assign({}, process.env), { RUBYOPT }) };
                ...
                Object.keys(preview_attributes).forEach((key) => {
                    if (typeof preview_attributes[key] === "string") {
                        var value = preview_attributes[key];
                        if (workspacePath !== undefined) {
                ...
                var asciidoctor = child_process_1.spawn(adoc_cmd, adoc_cmd_args, options);
                ...
                asciidoctor.stdout.on('data', (data) => {
                    result_data = Buffer.concat([result_data, data]);
                });
    ...
    fixLinks(html) {
        let result = html.replace(new RegExp("((?:src|href)=[\'\"])(?!(?:http:|https:|ftp:|#))(.*?)([\'\"])", "gmi"), (subString, p1, p2, p3) => {
            return [
                p1,
                fileUrl(path.join(path.dirname(this.filename), p2)),
                p3,
            ].join("");
        });
        return result;
    }
    parseText(text, forHTMLSave = false, backend = 'html') {
        return __awaiter(this, void 0, void 0, function* () {
